...
- [ ] Animated kitty graphics CPU usage — `top` shows ~30% CPU per visible animated image. Investigate: is `tickAnimations()` doing per-frame work it doesn't need to (e.g. reuploading whole frames every tick instead of advancing a frame index)? Are we re-issuing GPU image-vertex uploads each frame even when only the displayed frame index changed? Profile with one then two animated images to see if it scales linearly.
