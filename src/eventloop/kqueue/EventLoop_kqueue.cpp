...
int KQueueEventLoop::retainDirWatch(const std::string &dir)
{
    ...
    struct kevent ev {};
    EV_SET(&ev, fd, EVFILT_VNODE, EV_ADD | EV_ENABLE | EV_CLEAR, NOTE_WRITE, 0, nullptr);
    kevent(kqFd_, &ev, 1, nullptr, 0, nullptr);
...
void KQueueEventLoop::recheckEntry(WatchEntry &w)
{
    ...
    if (newMtime == w.lastMtime && newSize == w.lastSize) {
        return;
    }
...
{
    for (auto &w : fileWatches_) {
        releaseDirWatch(w.dir);
    }
    fileWatches_.clear();
