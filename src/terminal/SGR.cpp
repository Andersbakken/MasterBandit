        ...
        switch (p) {
            ...
            case 4: { // Underline (with optional style sub-param)
                ...
                if (style == 0) {
                    ...
                    switch (style) {
                        case 1:
                        case -1: mState->currentAttrs.setUnderlineStyle(0); break; // straight (default)
            ...
            case 22:
                mState->currentAttrs.setBold(false);
                mState->currentAttrs.setDim(false);
                break;
            ...
            case 58: // Underline color
                if (params[i].subCount >= 2 && params[i].subs[0] == 5) {
                    ...
                    uint8_t r, g, b;
                    color256ToRGB(params[i].subs[1], r, g, b);
                    mState->currentUnderlineColor = static_cast<uint32_t>(r) | (static_cast<uint32_t>(g) << 8) | (static_cast<uint32_t>(b) << 16) | 0xFF000000u;
                } else if (params[i].subCount >= 4 && params[i].subs[0] == 2) {
                    // Colon form: 58:2:CS:R:G:B or 58:2:R:G:B
                    int off                       = params[i].subCount >= 5 ? 2 : 1;
                    mState->currentUnderlineColor = static_cast<uint32_t>(params[i].subs[off] & 0xFF) | (static_cast<uint32_t>(params[i].subs[off + 1] & 0xFF) << 8) | (static_cast<uint32_t>(params[i].subs[off + 2] & 0xFF) << 16) | 0xFF000000u;
                } else if (i + 1 < params.size()) {
                    ...
                    if (params[i + 1].value == 5 && i + 2 < params.size()) {
                        ...
                        mState->currentUnderlineColor = static_cast<uint32_t>(params[i + 2].value & 0xFF) | (static_cast<uint32_t>(params[i + 3].value & 0xFF) << 8) | (static_cast<uint32_t>(params[i + 4].value & 0xFF) << 16) | 0xFF000000u;
                        i += 4;
                    }
                }
                break;
