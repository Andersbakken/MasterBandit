...
- [ ] Divider tint on focus loss — `updateDividerViewport` calls `queue.WriteBuffer` and currently runs on the main thread (resize, tab switch). These need to be moved to the render thread to avoid racing with `drawDivider`. Once on the render thread, the divider uniform buffer can be updated with the window focus tint on `focusChanged`, matching the tab bar and pane content dimming behavior.
