...
struct CaptureTestTerminal
{
    ...
    CaptureTestTerminal()
    {
        ...
        tmpDir = fs::temp_directory_path() / ("mb-cap-test-" + std::to_string(::getpid()) + "-" + std::to_string(reinterpret_cast<uintptr_t>(this)));
...
TEST_CASE("Text capture: strips DCS sequences (ESC P ... ESC \\)")
{
    ...
    const char data[] =
        "before"
        "\x1b"
        "Pq#0;2;100;0;0!~\a~~~\x1b\\" // sixel-ish gibberish with embedded BEL
        "after\n";
    t.term->deliverCapturedOutput(data, sizeof(data) - 1);
...
TEST_CASE("Multiple captures: same delivery reaches all of them")
{
    ...
    CHECK(CaptureTestExposed::readFile(pc).find("payload") != std::string::npos);
...
TEST_CASE("Auto-stop on write failure: ENOSPC triggers stopped event" * doctest::skip(::access("/dev/full", W_OK) != 0))
{
    CaptureTestExposed t;
...
TEST_CASE("Terminal destructor closes captures silently (no stopped event)")
{
    ...
    fs::path persistent =
        fs::temp_directory_path() / ("mb-cap-dtor-" + std::to_string(::getpid()));
...
TEST_CASE("Concurrent deliverCapturedOutput from multiple threads serialises writes")
{
    ...
    for (char c : got) {
        if (c == 'A') {
            ...
            FAIL("unexpected byte in capture: ", static_cast<int>(c));
        }
    }
