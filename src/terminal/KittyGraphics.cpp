...
bool parseCommand(std::string_view control, KittyGraphicsCommand &cmd)
{
    ...
    while (pos < control.size()) {
        ...
        if (pos >= control.size() || control[pos] != '=') {
            return false;
        }
        pos++; // skip '='
...
bool TerminalEmulator::setImageFrameShownAtForTest(uint32_t id, uint64_t t)
{
    ...
    if (it == mImageRegistry.end() || !it->second) {
        return false;
    }
...
void TerminalEmulator::processAPC(std::string_view body)
{
    ...
    auto sendResponse = [&](uint32_t imgId, const char *msg, bool dataLoaded = true)
    {
        ...
        if (cmd.imageNumber > 0) {
            pos += snprintf(buf + pos, sizeof(buf) - pos, "%sI=%u", pos > 3 ? "," : "", cmd.imageNumber);
        }
    ...
    auto resolveId = [&]() -> uint32_t
    {
        ...
        if (cmd.imageNumber > 0) {
            return findImageByNumber(cmd.imageNumber);
        }
    ...
    switch (cmd.action) {
        ...
        case 'p': {
            ...
            pl.cropX                        = cmd.xOffset;
            pl.cropY                        = cmd.yOffset;
            pl.cropW                        = cmd.width;
            pl.cropH                        = cmd.height;
        ...
        case 'd': // delete
        {
            ...
            switch (da) {
                ...
                case 'c':
                case 'C': // delete at cursor position
                case 'p':
                case 'P': // delete at cell position (x=, y=)
                {
                ...
                case 'q':
                case 'Q': // delete by position + z-index
                {
                    int targetCol   = cmd.xOffset > 0 ? static_cast<int>(cmd.xOffset) - 1 : 0;
                    int targetRow   = cmd.yOffset > 0 ? static_cast<int>(cmd.yOffset) - 1 : 0;
                    int32_t targetZ = cmd.zIndex;
                    ...
                    for (int c = 0; c < mWidth; c++) {
                        ...
                        if (plIt != imgIt->second->placements.end()) {
                            if (plIt->second.cellWidth > 0) {
                                cw = plIt->second.cellWidth;
                            }
                            if (plIt->second.zIndex != targetZ) {
                                continue;
                            }
                        } else if (targetZ != 0) {
                            continue;
        ...
        case 'f': // animation frame load
        {
            ...
            auto decoded = decodeImageData(chunkData, cmd.compressed, cmd.format, cmd.dataWidth, cmd.dataHeight);
            ...
            if (isNewFrame) {
                ...
                if (frameNumber == 1) {
                    ...
                    baseRGBA = img.extraFrames[frameNumber - 2].rgba;
                }
            }
            ...
            if (fastStore) {
                ...
                for (int y = 0; y < srcH; y++) {
                    int dy = oy + y;
                    if (dy < 0 || dy >= static_cast<int>(img.pixelHeight)) {
                        continue;
                    }
