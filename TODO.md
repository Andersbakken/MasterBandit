...
Two-thread model: main thread owns the event loop, PTY reads, VT parsing, window/input, and script execution; a dedicated render thread owns GPU submission. Render reads from `TerminalSnapshot` (viewport copy) under `TerminalEmulator::mMutex` (`std::recursive_mutex`). Wakeups via `renderCv_` / `renderWake_`; main thread defers structural mutations (tab/pane/terminal teardown, config reload) via `deferIfRendering()` keyed on the `renderActive_` atomic. See `src/platform/PlatformDawn.{h,cpp}`, `src/platform/Platform_Render.cpp`, and `src/terminal/TerminalSnapshot.{h,cpp}` for the concrete implementation.
...
- [ ] Config reload can invalidate render caches mid-frame — the config file watcher fires `applyConfig()` on the main thread under `platformMutex_`, which can rebuild font/color/atlas state while the render thread is in its shaping-phase unlock window holding raw `Pane*`/`Terminal*` captures. Debounced at 100 ms so collisions are rare but not impossible, and the resulting UB is silent. Fix options: (a) snapshot the config into the frame at frame start and have the render thread read only from the snapshot; (b) route `applyConfig()` through `deferIfRendering()` so it waits for `renderActive_` to clear before touching shared caches. (a) is cleaner long-term; (b) is a one-line mitigation.
