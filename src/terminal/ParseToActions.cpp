    ...
    for (; i < len; ++i) {
        switch (mParserState) {
                ...
                switch (mEscapeBuffer[0]) {
                    ...
                    case ST:
                        if (mWasInStringSequence) {
                            emit(out, StringSequence { mStringSequenceType, std::move(mStringSequence) });
                        ...
                        if (mEscapeIndex > 1) {
                            if (buf[i] >= 0x40 && buf[i] <= 0x7e) {
                                ...
                                int sync = detectSyncOutputCsi(
                                    mEscapeBuffer + 1,
                                    mEscapeIndex - 1);
                                std::memcpy(csi.buf.data(), mEscapeBuffer, static_cast<size_t>(mEscapeIndex));
                    ...
                    case ')': // G1 charset designation — ESC ) X
                        if (mEscapeIndex >= 2) {
                            emit(out, DesignateCharset { mEscapeBuffer[0], mEscapeBuffer[1] });
                            resetEscape();
                        }
            ...
            case InStringSequence:
                if (buf[i] == '\x07') {
                    // BEL terminator
                    emit(out, ParserAction::StringSequence { mStringSequenceType, std::move(mStringSequence) });
                    mStringSequence.clear();
                    resetEscape();
                } else if (buf[i] == 0x1b) {
