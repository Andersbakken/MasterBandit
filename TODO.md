...
- [ ] Scripting: TypeScript definitions — `types/mb.d.ts` covers `mb` global, `Pane`/`Popup`/`Tab`/`Overlay`, all object-level events, permissions, and the `mb:ws` module (server API). Still TODO: `mb:fs` and `mb:tui` module declarations. Also: generate this from a single source of truth (e.g. JSON-Schema-of-API derived from `ScriptEngine.cpp`) so the `.d.ts` can't drift from the native bindings.
