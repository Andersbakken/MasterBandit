                         ...
                         void * /*user*/, void *in, size_t len)
{
    ...
    switch (reason) {
        ...
        case LWS_CALLBACK_DEL_POLL_FD: {
            auto *pa = static_cast<struct lws_pollargs *>(in);
            auto it  = std::find_if(st->pollfds.begin(), st->pollfds.end(), [pa](const struct pollfd &p)
                                   {
                                       return p.fd == pa->fd;
                                   });
            if (it != st->pollfds.end()) {
                st->pollfds.erase(it);
            }
            break;
        }
...
int runCLI(int argc, char **argv)
{
    ...
    opts.add_options()("ctl", "IPC client mode")("p,pid", "Connect to specific MasterBandit PID", cxxopts::value<std::string>()->default_value(""))("s,socket", "Connect to specific socket path", cxxopts::value<std::string>()->default_value(""))("t,target", "Screenshot target (pane name or id)", cxxopts::value<std::string>()->default_value(""))("c,cell", "Screenshot cell rect: x,y,w,h", cxxopts::value<std::string>()->default_value(""))("f,format", "Screenshot format: png | grid", cxxopts::value<std::string>()->default_value("png"))("h,help", "Print usage")("command", "Command to run", cxxopts::value<std::string>())("args", "Command arguments", cxxopts::value<std::vector<std::string>>()->default_value(""));
    ...
    if (command == "screenshot") {
        ...
        if (positionalArgs.empty()) {
            fprintf(stderr, "inject requires text\n");
            return 1;
        }
        reqObj["cmd"]  = "inject";
        ...
        if (positionalArgs.empty()) {
            fprintf(stderr, "action requires a name\n");
            return 1;
        }
        reqObj["cmd"]    = "action";
        ...
        if (positionalArgs.size() >= 2 && !positionalArgs[1].empty()) {
            try {
                reqObj["repeat"] = std::stod(positionalArgs[1]);
        ...
        if (positionalArgs.size() >= 1) {
            double v = 0;
            if (parseNum(positionalArgs[0], v)) {
                reqObj["timeout_ms"] = v;
            }
        }
        if (positionalArgs.size() >= 2) {
            double v = 0;
            if (parseNum(positionalArgs[1], v)) {
                reqObj["settle_ms"] = v;
            }
        }
    } else {
