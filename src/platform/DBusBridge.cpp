...
{
    uint32_t m = 0;
    if (dbusFlags & DBUS_WATCH_READABLE) {
        m |= EPOLLIN;
    }
    if (dbusFlags & DBUS_WATCH_WRITABLE) {
        m |= EPOLLOUT;
    }
    return m;
}
        ...
        outbox_.push_back([this, msg, cb = std::move(cb)]() mutable
                          {
                              ...
                              if (!dbus_pending_call_set_notify(pending,
                                                                &DBusBridge::s_pending_notify,
                                                                heapCb,
                                                                [](void *p)
                                                                {
                                                                    delete static_cast<ReplyCb *>(p);
                                                                })) {
...
int DBusBridge::msUntilNextTimeout()
{
    ...
    for (auto &kv : timeouts_) {
        ...
        if (kv.second.deadlineNs < soonest) {
            soonest = kv.second.deadlineNs;
        }
    }
...
dbus_bool_t DBusBridge::s_add_watch(DBusWatch *w, void *data)
{
    ...
    if (fd < 0) {
        fd = dbus_watch_get_socket(w);
    }
    if (fd < 0) {
        return TRUE; // nothing to register, but not an OOM
    }
