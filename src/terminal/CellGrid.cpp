...
{
    if (top < 0 || bottom > rows_ || top >= bottom || n <= 0) {
        return;
    }
...
CellExtra &CellGrid::ensureExtra(int col, int row)
{
    ...
    if (!ex) {
        ex = std::make_unique<std::unordered_map<int, CellExtra>>();
    }
    return (*ex)[col];
}
