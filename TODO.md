...
- [x] Divider tint on focus loss — `updateDividerViewport` and `updateDividerBuffer` now run on the render thread only (in `Platform_Render.cpp`). Divider uniform buffer updates with window focus tint on `focusChanged`.
...
- [ ] OSC 133: preserve D exit code in tier-2 archive. Today `D;<exit>` is parsed and stored on `CommandRecord::exitCode` but the ring is bounded at 256 entries — older commands keep their cell tags in tier-2 but lose their exit code. Serialize `\e]133;D;<exit>\e\\` at the boundary cell of a completed output zone so archive re-reads can reconstruct a minimal `{absRow, exitCode}` side-table. Motivating use case: iTerm2-style gutter arrows coloring old output zones by success/failure. Also: "what was the exit code of the command I ran 400 scrollbacks ago." Ring-growth (256 → 10k) is an alternative, but bounded.
...
- [ ] Extract subsystems from `PlatformDawn`. The `Platform_*.cpp` files (EventLoop, Input, Actions, Render, Tabs, TabBar, Debug) are all `PlatformDawn::*` method implementations — a file split pretending to be a class decomposition. Every extension point reaches into `platformMutex_` / `renderActive_` / the deferred-mutation queues, so adding a new subsystem couples it to every other one. Promote each group into a real class owned by `PlatformDawn` (similar to how `Renderer` / `TexturePool` / `ComputeStatePool` already work): `RenderOrchestrator` (snapshot → resolveRow → frame dispatch), `InputRouter` (key/mouse + action dispatch), `TabManager` (tab/pane lifecycle, dividers, tab bar), `EventLoopHost` (tick, timers, PTY polling), `DebugSurface` (IPC handlers). `PlatformDawn` becomes a thin composition root owning device + window + subsystems. Before starting, pin down the locking convention — does each subsystem take `platformMutex_` itself, or is it called under the lock by its owner? — otherwise the extraction just relocates the coupling.
...
Main thread accumulates mutations in `PendingMutations pending_` (no lock needed — main-thread-only), then applies them in a single batch via `applyPendingMutations()` under `platformMutex_` at the end of each event loop tick. This rebuilds `RenderFrameState renderState_` — a shadow copy of tab/pane structure, font metrics, colors, divider geometry, and pre-computed tab bar cells. The render thread snapshots `renderState_` into `frameState_` (render-thread-only) under the same mutex at frame start, then releases the lock for the remainder of the frame. All post-unlock reads go through `frameState_`; render-thread-only state lives in `PaneRenderPrivate` maps. Zero GPU/Dawn calls on the main thread after init. See `src/platform/RenderSync.h`, `src/platform/PlatformDawn.{h,cpp}`, `src/platform/Platform_Render.cpp`, and `src/terminal/TerminalSnapshot.{h,cpp}`.
...
- [x] Config reload can invalidate render caches mid-frame — fixed: `reloadConfigNow()` holds `platformMutex_` for its duration so font/color/atlas mutations can't overlap with the render thread's shaping phase. Render thread reads config values from `frameState_` (snapshotted under lock).
- [ ] `platformMutex_` ownership assertion wrapper — wrap `platformMutex_` in a class that tracks the owning `std::thread::id` (set after lock, cleared before unlock). Add an `assertHeld()` method that asserts `owner == std::this_thread::get_id()`. Call it at the top of functions like `applyFramebufferResize()`, `buildRenderFrameState()`, `refreshDividers()` that expect the caller to already hold the lock. Debug-only (no-op in release builds).
