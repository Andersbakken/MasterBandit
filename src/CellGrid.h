...
struct CellAttrs {
    ...
    // Pack fg as RGBA8 for GPU tint (alpha = 0xFF)
    uint32_t packFgAsU32() const {
        if (fgMode() == Default) return 0xFFFFFFFF; // white
    ...
    // Pack bg as RGBA8 for GPU (alpha = 0 means default/transparent)
...
struct Cell {
    char32_t wc = 0;     // codepoint (0 = empty)
    CellAttrs attrs{};
};
...
public:
    ...
    Cell& cell(int col, int row) { return cells_[row * cols_ + col]; }
    const Cell& cell(int col, int row) const { return cells_[row * cols_ + col]; }
    Cell* row(int row) { return &cells_[row * cols_]; }
    const Cell* row(int row) const { return &cells_[row * cols_]; }
    ...
    // Row operations (all mark dirty automatically)
    void clearRow(int row);
    void clearRow(int row, int startCol, int endCol);
    void scrollUp(int top, int bottom, int n);
    void scrollDown(int top, int bottom, int n);
    void deleteChars(int row, int col, int count);
    void insertChars(int row, int col, int count);

private:
    int cols_ = 0, rows_ = 0;
    std::vector<Cell> cells_;      // flat: rows * cols
