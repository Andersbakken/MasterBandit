                                        ...
                                        int viewportOffset, int viewportRows)
{
    ...
    for (const auto &a : anchors) {
        ...
        if (viewRow < 0 || viewRow >= viewportRows) {
            continue;
        }
        out.push_back({ viewRow, a.rows, a.lineId });
    }
    std::sort(out.begin(), out.end(), [](const ViewAnchor &a, const ViewAnchor &b)
              {
...
{
    if (mUsingAltScreen) {
        return screenRow; // alt screen has no scrollback
    }
...
{
    if (!mCommandInProgress || mCommandRing.empty()) {
        return nullptr;
    }
...
void TerminalEmulator::startCommand(int absRow, int col)
{
    ...
    if (CommandRecord *existing = inProgressCommandMut()) {
        if (existing->outputStartCol < 0) {
            ...
            existing->commandStartLineId = 0;
            existing->commandStartCol    = -1;
            return;
        }
...
const TerminalEmulator::CommandRecord *TerminalEmulator::commandForLineId(uint64_t lineId) const
{
    auto it = std::upper_bound(
        ...
        mCommandRing.end(),
        lineId,
        [](uint64_t id, const CommandRecord &r)
        {
            return id < r.promptStartLineId;
        });
    if (it == mCommandRing.begin()) {
        return nullptr;
    }
    --it;
...
const TerminalEmulator::CommandRecord *TerminalEmulator::commandForId(uint64_t commandId) const
{
    ...
    auto it = std::lower_bound(
        ...
        mCommandRing.end(),
        commandId,
        [](const CommandRecord &r, uint64_t id)
        {
            return r.id < id;
        });
    if (it == mCommandRing.end() || it->id != commandId) {
        return nullptr;
    }
    return &*it;
}
...
void TerminalEmulator::pruneCommandRing()
{
    ...
    while (!mCommandRing.empty()) {
        ...
        if (mDocument.firstAbsOfLine(promptId) >= 0) {
            break;
        }
...
void TerminalEmulator::setSelectedCommand(std::optional<uint64_t> commandId)
{
    ...
    if (mSelectedCommandId == commandId) {
        return;
    }
...
std::string TerminalEmulator::serializeScrollback() const
{
    ...
    for (int i = 0; i < histSize + mHeight; ++i) {
        ...
        for (int c = 0; c < effectiveWidth; ++c) {
            ...
            if (ex) {
                for (char32_t ccp : ex->combiningCps) {
                    emitCp(ccp);
                }
            }
        }
...
void TerminalEmulator::processCSI(const char *buf, int len)
{
    ...
    switch (finalByte) {
        ...
        case DSR:
            if (isPrivate && len >= 4) {
                ...
                if (ps == 996) {
                    ...
                    bool isDark = true;
                    if (mCallbacks.isDarkMode) {
                        isDark = mCallbacks.isDarkMode();
                    }
                    char response[16];
                ...
                char response[32];
                int rlen = snprintf(response, sizeof(response), "\x1b[%d;%dR", reportY + 1, mState->cursorX + 1);
                writeToOutput(response, rlen);
            } else {
        ...
        case 't': {
            ...
            if (*end == ';') {
                ps = static_cast<int>(parseUL(end + 1, &end));
            }
        ...
        case 'p': {
            ...
            if (isPrivate && len >= 5 &&
                ...
                char response[32];
                int rlen = snprintf(response, sizeof(response), "\x1b[?%d;%d$y", ps, pm);
                writeToOutput(response, rlen);
...
void TerminalEmulator::restorePrivateModes(const std::vector<int> &modes)
{
    ...
    if (modes.empty()) {
        for (auto &kv : mSavedPrivateModes) {
            restoreOne(kv.first);
        }
    } else {
        for (int m : modes) {
