...
void TerminalEmulator::processOSC_iTerm(std::string_view payload)
{
    ...
    while (pos < paramStr.size()) {
        ...
        if (key == "inline" && val == "1") {
            ...
            widthSpec = parseITermDim(val);
        } else if (key == "height") {
            heightSpec = parseITermDim(val);
        } else if (key == "preserveAspectRatio") {
            preserveAspect = (val != "0");
        } else if (key == "name") {
            nameB64 = val;
        }
    ...
    switch (oscNum) {
                ...
                if (!tok.empty()) {
                    ...
                    if (eq == std::string_view::npos) {
                        if (tokenIndex == 0) {
                            ...
                            try {
                                firstArgInt = std::stoi(std::string(tok));
                            } catch (...) {
                            }
                        }
                    } else {
        ...
        case 9: {
            ...
            if (payload.substr(0, 2) == "4;") {
                ...
                int state = 0, pct = 0;
                try {
                    state = std::stoi(std::string(stateStr));
                } catch (...) {
                }
                if (!pctStr.empty()) {
                    try {
                        pct = std::stoi(std::string(pctStr));
                    } catch (...) {
                    }
                }
                if (mCallbacks.onProgressChanged) {
                    mCallbacks.onProgressChanged(state, pct);
                }
        ...
        case 99: // Desktop notifications (kitty protocol)
        {
            ...
            while (!sv.empty()) {
                ...
                if (key == "i") {
                    ...
                    char c = val[0];
                    if (c >= '0' && c <= '2') {
                        mNotifyUrgency = static_cast<uint8_t>(c - '0');
                    }
                } else if (key == "c") {
                    ...
                    while (!list.empty()) {
                        ...
                        if (tok == "focus") {
                            mNotifyActionFocus = !remove;
                        } else if (tok == "report") {
                            mNotifyActionReport = !remove;
                        }
        ...
        default:
            if (mCallbacks.onOSC) {
                mCallbacks.onOSC(oscNum, payload);
            } else {
                spdlog::warn("Ignoring OSC {}", oscNum);
            }
            break;
    }
}
...
void TerminalEmulator::processOSC_Clipboard(std::string_view payload)
{
    ...
    if (!wantClipboard && !wantPrimary) {
        wantClipboard = true;
    }

    if (data == "?") {
        if (!mCallbacks.pasteFromClipboard) {
            return;
        }
        ...
        std::string encoded      = base64::encode(
            reinterpret_cast<const uint8_t *>(clip.data()),
            clip.size());
        std::string destEcho;
        if (wantClipboard) {
            destEcho += 'c';
...
void TerminalEmulator::processOSC_Color(int oscNum, std::string_view payload)
{
    ...
    switch (oscNum) {
        case 10:
            r = &mDefaultColors.fgR;
            g = &mDefaultColors.fgG;
            b = &mDefaultColors.fgB;
            break;
        case 11:
            r = &mDefaultColors.bgR;
            g = &mDefaultColors.bgG;
            b = &mDefaultColors.bgB;
            break;
        case 12:
            r = &mDefaultColors.cursorR;
            g = &mDefaultColors.cursorG;
            b = &mDefaultColors.cursorB;
            break;
        default: return;
    }
    ...
    if (payload == "?") {
        ...
        char response[64];
        int len = snprintf(response, sizeof(response), "\x1b]%d;rgb:%04x/%04x/%04x\x1b\\", oscNum, static_cast<unsigned>(*r) * 257, static_cast<unsigned>(*g) * 257, static_cast<unsigned>(*b) * 257);
...
void TerminalEmulator::processOSC_Palette(std::string_view payload)
{
    ...
    for (size_t i = 0; i + 1 < tokens.size(); i += 2) {
        ...
        if (spec == "?") {
            ...
            char response[64];
            int len = snprintf(response, sizeof(response), "\x1b]4;%d;rgb:%04x/%04x/%04x\x1b\\", idx, static_cast<unsigned>(r) * 257, static_cast<unsigned>(g) * 257, static_cast<unsigned>(b) * 257);
            writeToOutput(response, len);
        } else {
