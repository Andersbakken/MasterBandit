                    ...
                    uint32_t width, uint32_t height)
{
    ...
    // Rect uniform buffer (viewport)
    {
        ...
        float uniforms[12] = {
            ...
            1.0f,
            1.0f,
            1.0f, // pane_tint default = no tint
            1.0f,
            0.0f,
            0.0f,
            0.0f // dim_params: factor=1 is identity → no-op multiply
        };
                               ...
                               const FontData &font)
{
    ...
    uint64_t version = font.atlasVersion.load(std::memory_order_acquire);
    if (version == gpu.uploadedVersion) {
        return;
    }
    ...
    queue.WriteBuffer(gpu.storageBuffer, offset, font.atlasData.data() + startStorageTexel * 4, size);
    gpu.uploadedSize    = font.atlasUsed;
    gpu.uploadedVersion = version;
}
...
{
    if (!computeInitialized_ || !state || count == 0) {
        return;
    }
    queue.WriteBuffer(state->resolvedCellBuffer, 0, cells, static_cast<uint64_t>(count) * sizeof(ResolvedCell));
}
                            ...
                            const GlyphEntry *glyphs, uint32_t count)
{
    ...
    queue.WriteBuffer(state->glyphBuffer, 0, glyphs, static_cast<uint64_t>(count) * sizeof(GlyphEntry));
}
                            ...
                            size_t rangeStart, size_t rangeCount)
{
    ...
    float uniforms[12] = { paneWidth, paneHeight, 0, 0, tint[0], tint[1], tint[2], tint[3], dim.factor, dim.yMin, dim.yMax, 0.0f };
                            ...
                            size_t imgSplitText)
{
    ...
    // Update uniforms — use full pane viewport (including padding) for NDC transform
    {
        ...
        float uniforms[12] = { w, h, 0.0f, 0.0f, pane_tint[0], pane_tint[1], pane_tint[2], pane_tint[3], dim.factor, dim.yMin, dim.yMax, 0.0f };
        queue.WriteBuffer(rectUniformBuffer_, 0, uniforms, sizeof(uniforms));
                         ...
                         const std::vector<CompositeEntry> &entries)
{
    ...
    for (const auto &e : entries) {
        if (!e.texture || e.srcW == 0 || e.srcH == 0) {
            continue;
        }
        if (e.dstX >= swapW || e.dstY >= swapH) {
            continue;
        }

        uint32_t texW = e.texture.GetWidth();
        uint32_t texH = e.texture.GetHeight();
        if (e.srcX >= texW || e.srcY >= texH) {
            continue;
        }
        ...
        wgpu::Extent3D extent = { std::min({ e.srcW, maxSrcW, maxDstW }),
                                  std::min({ e.srcH, maxSrcH, maxDstH }),
                                  1 };
                                     ...
                                     const float *tint)
{
    ...
    float uniforms[12] = { static_cast<float>(fbWidth), static_cast<float>(fbHeight), 0.0f, 0.0f, tint[0], tint[1], tint[2], tint[3], 1.0f, 0.0f, 0.0f, 0.0f }; // dim_params: factor=1 = no dim
                                   ...
                                   float r, float g, float b, float a)
{
    ...
    auto set = [&](int i, float px, float py)
    {
        ...
        verts[i].color[2]     = b;
        verts[i].color[3]     = a;
        verts[i].edge_dist[0] = 1e3f;
        verts[i].edge_dist[1] = 1e3f;
    };
    set(0, x0, y0);
    set(1, x1, y0);
    set(2, x0, y1);
...
{
    if (!vb || !dividerBindGroup_ || !rectPipeline_) {
        return;
    }
    ...
    wgpu::RenderPassEncoder pass = encoder.BeginRenderPass(&rpDesc);
    pass.SetViewport(0.0f, 0.0f, static_cast<float>(fbWidth), static_cast<float>(fbHeight), 0.0f, 1.0f);
    pass.SetPipeline(rectPipeline_);
...
{
    if (bucket >= ColrAtlas::NUM_BUCKETS) {
        return;
    }
    ...
    if (bg.atlasDim == dim && bg.atlasDim != 0) {
        return;
    }
...
{
    if (!colrPipelineReady_ || cmds.empty()) {
        return;
    }
    ...
    // Upload glyph info buffer (single upload, sorted by bucket)
    {
        ...
        queue.WriteBuffer(colrGlyphInfoBuffer_, 0, sortedGlyphInfos.data(), sortedGlyphInfos.size() * sizeof(ColrGlyphInfoGPU));
    }
    ...
    // Upload instruction buffer
    {
        ...
        queue.WriteBuffer(colrInstructionBuffer_, 0, allInstructions.data(), allInstructions.size() * sizeof(uint32_t));
    }
    ...
    // Upload color stop buffer
    {
        ...
        queue.WriteBuffer(colrColorStopBuffer_, 0, allColorStops.data(), allColorStops.size() * sizeof(ColrColorStop));
    }
    ...
    while (rangeStart < totalGlyphs) {
        ...
        bgEntries[3].size                 = fontIt->second.uploadedSize > 0 ? (static_cast<uint64_t>(fontIt->second.uploadedSize) + 1) / 2 * 4 * sizeof(int32_t) : (static_cast<uint64_t>(fontIt->second.storageCapacity) + 1) / 2 * 4 * sizeof(int32_t);
...
{
    if (!imagePipelineReady_ || cmds.empty()) {
        return;
    }
    ...
    for (uint32_t bi = 0; bi < ColrAtlas::NUM_BUCKETS; bi++) {
        ...
        queue.WriteBuffer(colrQuadVertexBuffer_, 0, verts.data(), verts.size() * sizeof(ImageVertex));
