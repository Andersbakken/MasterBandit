...
- [ ] Alt screen state save/restore refactor — Factor all per-screen terminal state (cursor shape, blink, mouse modes, bracketed paste, focus reporting, cursor color, etc.) into a `TerminalState` struct. Save on `CSI ? 1049 h`, restore on `CSI ? 1049 l`. Currently only cursor position, attrs, wrap pending, cursor shape, and cursor blink are saved. Mouse modes, bracketed paste, focus reporting, and other DEC private modes are not restored — an app that crashes without cleanup leaves these enabled for the shell.
