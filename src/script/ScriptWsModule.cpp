...
static JSValue jsWsConnNew(JSContext *ctx, WsConnection *c)
{
    ...
    JS_SetPropertyFunctionList(ctx, obj, jsWsConnFuncs, sizeof(jsWsConnFuncs) / sizeof(jsWsConnFuncs[0]));
    return obj;
}
            ...
            if (lws_is_final_fragment(wsi) && srv->ctx && !JS_IsUndefined(c->jsObj)) {
                ...
                if (c->rxBinary) {
                    ...
                    arg = JS_NewStringLen(srv->ctx,
                                          reinterpret_cast<const char *>(c->rxBuffer.data()),
                                          c->rxBuffer.size());
                }
...
static JSValue jsWsCreateServer(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (!inst) {
        return JS_ThrowTypeError(ctx, "mb:ws: no script context");
    }
    ...
    if (!srv->vhost) {
        return JS_ThrowTypeError(ctx, "mb:ws: failed to bind on %s:%d", srv->host.c_str(), port);
    }
    ...
    JS_SetPropertyFunctionList(ctx, obj, jsWsServerFuncs, sizeof(jsWsServerFuncs) / sizeof(jsWsServerFuncs[0]));
...
static int jsWsModuleInit(JSContext *ctx, JSModuleDef *m)
{
    ...
    JS_SetPropertyStr(ctx, wsObj, "createServer", JS_NewCFunction(ctx, jsWsCreateServer, "createServer", 1));
    JS_SetModuleExport(ctx, m, "default", wsObj);
    JS_SetModuleExport(ctx, m, "createServer", JS_NewCFunction(ctx, jsWsCreateServer, "createServer", 1));
    return 0;
}
