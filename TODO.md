...
- [ ] Script callbacks during parsing — `TerminalCallbacks` fire from `TerminalEmulator::parseBatch()` on the main thread (title, icon, CWD, progress, OSC handlers, `commandComplete`, `foregroundProcessChanged`, etc.), re-entering QuickJS under `TerminalEmulator::mMutex`. A `deferredMain_` / `postToMainThread()` drain already exists and some lifecycle callbacks route through it, but the parse-time OSC and command-lifecycle callbacks still run synchronously. A blocking, long-running, or recursively-injecting script stalls the PTY read loop and can deadlock any path that expects to take `mMutex` from a callback. Route the remaining parse-time callbacks through the post-parse drain so `mMutex` is released before JS runs; consider a per-script wall-time budget for the drain to bound tail latency.
