...
JSValue jsLayoutSetRoot(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "setRoot: expected a UUID");
    }
...
JSValue jsLayoutAppendChild(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    ChildSlot slot;
    if (argc >= 3) {
        if (!parseChildSlot(ctx, argv[2], child, slot)) {
            return JS_EXCEPTION;
        }
    } else {
        slot.id = child;
...
JSValue jsLayoutRemoveChild(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "removeChild(parent, child)");
    }
    Uuid parent = parseUuidArg(ctx, argv[0], "removeChild(parent)");
    if (parent.isNil()) {
        return JS_EXCEPTION;
    }
    Uuid child = parseUuidArg(ctx, argv[1], "removeChild(child)");
    if (child.isNil()) {
        return JS_EXCEPTION;
    }
...
JSValue jsLayoutSetActiveChild(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "setActiveChild(stack, child)");
    }
    Uuid stack = parseUuidArg(ctx, argv[0], "setActiveChild(stack)");
    if (stack.isNil()) {
        return JS_EXCEPTION;
    }
    Uuid child = parseUuidArg(ctx, argv[1], "setActiveChild(child)");
    if (child.isNil()) {
        return JS_EXCEPTION;
    }
...
JSValue jsLayoutSetTabBarStack(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "setTabBarStack(tabBar, stackOrNull)");
    }
    Uuid bar = parseUuidArg(ctx, argv[0], "setTabBarStack(tabBar)");
    if (bar.isNil()) {
        return JS_EXCEPTION;
    }
...
JSValue jsLayoutSetLabel(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "setLabel(id, label)");
    }
    Uuid id = parseUuidArg(ctx, argv[0], "setLabel(id)");
    if (id.isNil()) {
        return JS_EXCEPTION;
    }
    size_t len    = 0;
...
JSValue jsLayoutDestroyNode(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "destroyNode(id)");
    }
    Uuid id = parseUuidArg(ctx, argv[0], "destroyNode(id)");
    if (id.isNil()) {
        return JS_EXCEPTION;
    }
...
JSValue jsLayoutActivateTab(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "activateTab(nodeId)");
    }
    ...
    auto *eng = engineFromCtx(ctx);
    if (eng->callbacks().activateTab) {
        eng->callbacks().activateTab(sub);
    }
    return JS_UNDEFINED;
}
...
JSValue jsLayoutFocusPane(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "focusPane(nodeId)");
    }
    auto *eng   = engineFromCtx(ctx);
...
JSValue jsLayoutRemoveNode(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "removeNode(nodeId)");
    }
    auto *eng   = engineFromCtx(ctx);
...
JSValue jsLayoutKillTerminal(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "killTerminal(nodeId)");
    }
    auto *eng   = engineFromCtx(ctx);
...
JSValue jsLayoutCloseTab(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "closeTab(nodeId)");
    }
    ...
    auto *eng = engineFromCtx(ctx);
    if (eng->callbacks().closeTab) {
        eng->callbacks().closeTab(sub);
    }
    return JS_TRUE;
}
...
JSValue jsLayoutCreateTerminalComposite(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "createTerminal(parentNodeId, opts?)");
    }
...
JSValue jsLayoutSplitPane(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "splitPane(existingNodeId, dir, opts?)");
    }
    ...
    if (dir == "horizontal" || dir == "h") {
        ...
        return JS_ThrowTypeError(ctx, "splitPane: bad dir");
    }
                             ...
                             SlotField which, const char *sig)
{
    ...
    if (argc < 3) {
        return JS_ThrowTypeError(ctx, "%s", sig);
    }
    Uuid parent = parseUuidArg(ctx, argv[0], "parent");
    if (parent.isNil()) {
        return JS_EXCEPTION;
    }
    Uuid child = parseUuidArg(ctx, argv[1], "child");
    if (child.isNil()) {
        return JS_EXCEPTION;
    }
    int32_t value;
...
{
    return jsLayoutSetSlotField(ctx, argc, argv, SlotField::Stretch, "setSlotStretch(parent, child, stretch)");
}

JSValue jsLayoutSetSlotMinCells(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    return jsLayoutSetSlotField(ctx, argc, argv, SlotField::MinCells, "setSlotMinCells(parent, child, minCells)");
}

JSValue jsLayoutSetSlotMaxCells(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    return jsLayoutSetSlotField(ctx, argc, argv, SlotField::MaxCells, "setSlotMaxCells(parent, child, maxCells)");
}

JSValue jsLayoutSetSlotFixedCells(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    return jsLayoutSetSlotField(ctx, argc, argv, SlotField::FixedCells, "setSlotFixedCells(parent, child, fixedCells)");
}
...
JSValue jsLayoutAdjustPaneSize(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc < 3) {
        return JS_ThrowTypeError(ctx, "adjustPaneSize(paneNodeId, dir, amount)");
    }
    Uuid pane = parseUuidArg(ctx, argv[0], "paneNodeId");
    if (pane.isNil()) {
        return JS_EXCEPTION;
    }
    size_t len     = 0;
    ...
    JS_FreeCString(ctx, ds);
    int32_t amount;
    if (JS_ToInt32(ctx, &amount, argv[2]) != 0) {
        return JS_EXCEPTION;
    }
    auto *eng = engineFromCtx(ctx);
...
JSValue jsLayoutFocusedPane(JSContext *ctx, JSValueConst, int, JSValueConst *)
{
    ...
    for (const auto &t : all) {
        ...
        JS_SetPropertyStr(ctx, o, "tabNodeId", t.nodeId.empty() ? JS_NULL : JS_NewStringLen(ctx, t.nodeId.data(), t.nodeId.size()));
        return o;
    }
    return JS_NULL;
}
...
bool kindFromName(std::string_view name, NodeKind &out)
{
    if (name == "Terminal" || name == "terminal") {
        out = NodeKind::Terminal;
        return true;
    }
    if (name == "Container" || name == "container") {
        out = NodeKind::Container;
        return true;
    }
...
JSValue jsLayoutQueryNodes(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    JS_FreeCString(ctx, s);
    if (!ok) {
        return JS_ThrowTypeError(ctx, "queryNodes: unknown kind");
    }
...
{
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "node(id)");
    }
    Uuid id = parseUuidArg(ctx, argv[0], "node(id)");
    if (id.isNil()) {
        return JS_EXCEPTION;
    }
    ...
    JS_SetPropertyStr(ctx, o, "parent", n->parent.isNil() ? JS_NULL : uuidToJs(ctx, n->parent));

    std::visit([&](const auto &d)
               {
                   using T = std::decay_t<decltype(d)>;
                   if constexpr (std::is_same_v<T, ContainerData>) {
                       JS_SetPropertyStr(ctx, o, "direction", JS_NewString(ctx, d.dir == SplitDir::Horizontal ? "horizontal" : "vertical"));
                       JS_SetPropertyStr(ctx, o, "children", childListToJs(ctx, d.children));
                   } else if constexpr (std::is_same_v<T, StackData>) {
                       JS_SetPropertyStr(ctx, o, "children", childListToJs(ctx, d.children));
                       JS_SetPropertyStr(ctx, o, "activeChild", d.activeChild.isNil() ? JS_NULL : uuidToJs(ctx, d.activeChild));
                       JS_SetPropertyStr(ctx, o, "opaque", JS_NewBool(ctx, d.opaque));
                       JS_SetPropertyStr(ctx, o, "zoomTarget", d.zoomTarget.isNil() ? JS_NULL : uuidToJs(ctx, d.zoomTarget));
                   } else if constexpr (std::is_same_v<T, TabBarData>) {
                       JS_SetPropertyStr(ctx, o, "boundStack", d.boundStack.isNil() ? JS_NULL : uuidToJs(ctx, d.boundStack));
                   }
...
JSValue jsLayoutComputeRects(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv)
{
    ...
    if (argc >= 2 && JS_ToInt32(ctx, &cellW, argv[1]) != 0) {
        return JS_EXCEPTION;
    }
    if (argc >= 3 && JS_ToInt32(ctx, &cellH, argv[2]) != 0) {
        return JS_EXCEPTION;
    }
...
void installLayoutBindings(Engine &, JSContext *ctx, JSValue mb)
{
    ...
    JS_SetPropertyStr(ctx, layout, "createTerminal", JS_NewCFunction(ctx, jsLayoutCreateTerminalComposite, "createTerminal", 2));
    JS_SetPropertyStr(ctx, layout, "createContainer", JS_NewCFunction(ctx, jsLayoutCreateContainer, "createContainer", 1));
    JS_SetPropertyStr(ctx, layout, "createStack", JS_NewCFunction(ctx, jsLayoutCreateStack, "createStack", 0));
    JS_SetPropertyStr(ctx, layout, "createTabBar", JS_NewCFunction(ctx, jsLayoutCreateTabBar, "createTabBar", 0));
    ...
    JS_SetPropertyStr(ctx, layout, "setSlotFixedCells", JS_NewCFunction(ctx, jsLayoutSetSlotFixedCells, "setSlotFixedCells", 3));
    JS_SetPropertyStr(ctx, layout, "adjustPaneSize", JS_NewCFunction(ctx, jsLayoutAdjustPaneSize, "adjustPaneSize", 3));
    JS_SetPropertyStr(ctx, layout, "focusedPane", JS_NewCFunction(ctx, jsLayoutFocusedPane, "focusedPane", 0));

    JS_SetPropertyStr(ctx, layout, "setRoot", JS_NewCFunction(ctx, jsLayoutSetRoot, "setRoot", 1));
    JS_SetPropertyStr(ctx, layout, "getRoot", JS_NewCFunction(ctx, jsLayoutGetRoot, "getRoot", 0));
    ...
    JS_SetPropertyStr(ctx, layout, "destroyNode", JS_NewCFunction(ctx, jsLayoutDestroyNode, "destroyNode", 1));
