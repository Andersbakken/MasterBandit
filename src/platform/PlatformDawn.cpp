...
{
    if (!scriptEngine_.layoutTree().takeDirty()) {
        return;
    }
    for (Uuid sub : scriptEngine_.tabSubtreeRoots()) {
        resizeAllPanesInTab(sub);
    }
}
...
{
    if (!renderEngine_ || !renderEngine_->device()) {
        return;
    }
    ...
    if (!platformInitialized_) {
        ...
        if (!isHeadless()) {
            ...
            {
                inputController_->onCursorPos(x, y);
            };
            ...
            window_->onFocus = [this](bool focused)
            {
                ...
                setNeedsRedraw();
                if (auto t = activeTab()) {
                    if (Terminal *fp = scriptEngine_.focusedTerminalInSubtree(*t)) {
        ...
        if (!isHeadless()) {
            ...
            if (!testEmojiFontPath_.empty()) {
                ...
                if (!emojiData.empty()) {
                    ...
                    textSystem_.setEmojiFallback([emojiData](char32_t)
                                                 {
                                                     return emojiData;
                                                 });
...
void PlatformDawn::applyFontChange(const Config &config)
{
    ...
    auto font = textSystem_.getFont(fontName_);
    if (!font) {
        spdlog::error("Config reload: font re-registration failed");
        return;
    }
...
void PlatformDawn::applyConfig(const Config &config)
{
    ...
    if (animScheduler_) {
        animScheduler_->applyBlinkConfig(config.cursor.blink_rate, config.cursor.blink_fps);
    }
...
void PlatformDawn::adjustFontSize(float delta)
{
    ...
    if (newSize < 6.0f * contentScaleX_ || newSize > 72.0f * contentScaleX_) {
        return;
    }
    fontSize_ = newSize;
...
void PlatformDawn::initTabBar(const TabBarConfig &cfg)
{
    ...
    if (!fontData.empty()) {
        ...
        if (!nerdData.empty()) {
            fl.push_back(std::move(nerdData));
        }
    ...
    if (font) {
        ...
        if (tabBarCharWidth_ < 1.0f) {
            tabBarCharWidth_ = tabBarFontSize_ * 0.6f;
        }
    } else {
