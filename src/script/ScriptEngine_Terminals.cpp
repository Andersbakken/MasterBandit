...
{
    if (nodeId.isNil() || scopeRoot.isNil()) {
        return false;
    }
    ...
    for (Uuid tn : terminalNodes) {
        if (focusedTerminalNodeId() == tn) {
            setFocusedTerminalNodeId({});
        }
    }
...
Rect resolveSubtreeContentRect(Engine &eng, Uuid subtreeRoot)
{
    ...
    if (fbW == 0 || fbH == 0 || tree.root().isNil()) {
        return fallback;
    }
...
{
    return !layoutTree_->splitByWrapping(existingChildNodeId, dir, newChildNodeId, newIsFirst).isNil();
}

::Terminal *Engine::paneInSubtree(Uuid subtreeRoot, Uuid nodeId)
{
    if (subtreeRoot.isNil() || nodeId.isNil()) {
        return nullptr;
    }
...
Rect Engine::nodeRectInSubtree(Uuid subtreeRoot, Uuid nodeId) const
{
    ...
    auto rects = layoutTree_->computeRectsFrom(subtreeRoot, content, lastCellW(), lastCellH());
...
Uuid neighborInDir(const Engine &eng, Uuid subtreeRoot, Uuid from,
                   int dx, int dy)
{
    if (subtreeRoot.isNil() || from.isNil()) {
        return {};
    }
...
{
    if (subtreeRoot.isNil() || dividerPx <= 0) {
        return {};
    }
    auto liveTerminals = activePanesInSubtree(subtreeRoot);
    if (liveTerminals.size() < 2) {
        return {};
    }
    ...
    Rect content             = resolveSubtreeContentRect(const_cast<Engine &>(*this),
                                             subtreeRoot);
    const ::LayoutTree &tree = *layoutTree_;
    auto rects               = tree.computeRectsFrom(subtreeRoot, content, lastCellW(), lastCellH());
