                              ...
                              float baseSize)
{
    ...
    sLog().info("Registered font '{}': {} pre-encoded glyphs, atlas {} texels, baseSize={:.0f}, {} font(s)",
                name,
                static_cast<uint32_t>(font.glyphs.size()),
                font.atlasUsed,
                baseSize,
                font.hbFonts.size());
                                          ...
                                          const std::vector<uint8_t> &ttfData)
{
    ...
    if (blobHash == 0) {
        blobHash = 1; // 0 reserved for "no hash"
    }
                                     ...
                                     uint32_t targetTexels)
{
    ...
    {
        std::shared_lock lock(font.mutex);
        if (font.atlasUsed <= budgetTexels) {
            return false;
        }
    }
...
void TextSystem::tagFontStyle(const std::string &name, uint32_t fontIndex, FontStyle style)
{
    ...
    FontData &font = *it->second;
    if (fontIndex >= font.hbFonts.size()) {
        return;
    }
    ...
    // Try existing fonts under read lock
    {
        ...
        for (uint32_t fi = 0; fi < font.hbFonts.size(); fi++) {
            ...
            if (emojiPresentation) {
                ...
                if (nonColrFallback.glyphId == 0) {
                    nonColrFallback = { fi, gid };
                }
                                 ...
                                 FontStyle style)
{
    ...
    // 1b. BiDi analysis
    SBCodepointSequence cpSeq { SBStringEncodingUTF8, const_cast<char *>(text.c_str()), static_cast<SBUInteger>(text.size()) };
    ...
    for (const auto &br : bidiRuns) {
        ...
        for (const auto &sr : scriptRuns) {
            ...
            if (s < e) {
                segments.push_back({ s, e - s, br.level, sr.script });
            }
        }
    }
    ...
    for (const auto &seg : segments) {
        uint32_t fi = resolveSegment(font,
                                     reinterpret_cast<const uint8_t *>(text.c_str() + seg.offset),
                                     seg.length,
                                     style);
        ...
        hb_buffer_add_utf8(buf, text.c_str(), static_cast<int>(text.size()), static_cast<uint32_t>(seg.offset), static_cast<int>(seg.length));
        hb_buffer_set_direction(buf, (seg.level & 1) ? HB_DIRECTION_RTL : HB_DIRECTION_LTR);
        hb_buffer_set_script(buf, hb_script_from_iso15924_tag(static_cast<hb_tag_t>(SBScriptGetUnicodeTag(seg.script))));
        ...
        auto pushGlyph = [&](uint32_t idx)
        {
            ...
            if (gid == 0) {
                ...
                if (resolved.glyphId != 0) {
                    ...
                    allGlyphs.push_back({ glyphKey(glyphFi, gid), infos[idx].cluster, adv, 0, 0, seg.level });
                    return;
                }
            }
            ...
            allGlyphs.push_back({ glyphKey(glyphFi, gid), infos[idx].cluster, positions[idx].x_advance * fontPixelScale * scale, positions[idx].x_offset * fontPixelScale * scale, positions[idx].y_offset * fontPixelScale * scale, seg.level });
        };
...

    {
        ...
        for (size_t gi = 0; gi < allGlyphs.size(); gi++) {
            ...
            if (g.cluster > lineStartByte && g.cluster < text.size()) {
                ...
                if (brk == LINEBREAK_MUSTBREAK) {
                    lineRanges.push_back({ lineStartGlyph, gi - lineStartGlyph, lineStartByte, g.cluster - lineStartByte, lineWidth });
                    lineStartGlyph = gi;
            ...
            if (wrapWidth > 0 && lineWidth > wrapWidth && hasBreak) {
                lineRanges.push_back({ lineStartGlyph, lastBreakGlyph - lineStartGlyph, lineStartByte, lastBreakByte - lineStartByte, widthAtBreak });
        ...
        lineRanges.push_back({ lineStartGlyph, allGlyphs.size() - lineStartGlyph, lineStartByte, endByte - lineStartByte, lineWidth });
    }
    ...
    if (align != 0 && wrapWidth > 0) {
        for (size_t li = 0; li < resultLines.size(); li++) {
            float offset = 0.0f;
            if (align == 1) {
                offset = (wrapWidth - resultLines[li].width) * 0.5f;
            } else if (align == 2) {
                offset = wrapWidth - resultLines[li].width;
            }

            size_t endIdx = (li + 1 < resultLines.size())
                ? resultLines[li + 1].startIdx
                : result.glyphs.size();
                               ...
                               std::span<const std::pair<uint32_t, int>> byteToCell)
{
    ...
    for (const auto &sr : scriptRuns) {
        uint32_t fi = resolveSegment(font,
                                     reinterpret_cast<const uint8_t *>(text.c_str() + sr.offset),
                                     sr.length,
                                     style);
        ...
        hb_buffer_t *buf = hb_buffer_create();
        hb_buffer_add_utf8(buf, text.c_str(), static_cast<int>(text.size()), static_cast<uint32_t>(sr.offset), static_cast<int>(sr.length));
        hb_buffer_set_direction(buf, segmentRtl ? HB_DIRECTION_RTL : HB_DIRECTION_LTR);
        hb_buffer_set_script(buf, hb_script_from_iso15924_tag(static_cast<hb_tag_t>(SBScriptGetUnicodeTag(sr.script))));
        hb_buffer_set_language(buf, hb_language_get_default());
        ...
        for (uint32_t i = 0; i < count; i++) {
                    ...
                    if (btcIdx >= 0) {
                        ...
                        if (cellByteEnd - cellByteStart > static_cast<uint32_t>(consumed)) {
                            ...
                            if (resolved.glyphId != 0) {
                                ...
                                hb_buffer_add_utf8(emojiBuf, text.c_str(), static_cast<int>(text.size()), cellByteStart, static_cast<int>(cellByteEnd - cellByteStart));
                                ...
                                for (uint32_t j = 0; j < emojiCount; j++) {
                                    ...
                                    result.glyphs.push_back({ glyphKey(emojiFi, eGid), cluster, adv, emojiPositions[j].x_offset * emojiPixelScale * scale, emojiPositions[j].y_offset * emojiPixelScale * scale, false, segmentRtl });
                                    emittedGlyph = true;
                                }
            ...
            result.glyphs.push_back({ glyphKey(glyphFi, gid),
                                      infos[i].cluster,
                                      adv,
